...
namespace BizName.Studio.Data.UnitTests.Repositories.PartitionedRepo;

public class PartitionedRepositoryImplPrepareCosmosDocumentSimpleTests
{
    private readonly PartitionedRepositoryImpl<TestData.TestEntity> _repository = new(CreateMockDatabaseContext());
    private readonly Guid _tenantId = Guid.NewGuid();

    [Fact]
    public void Should_Add_TenantId_PartitionKey_To_Document()
    {
        // Arrange
        var entity = TestData.NewTestEntity;
        
        // Act
        var document = _repository.PrepareCosmosDocument(entity, _tenantId);
        
        // Assert
        document.Should().NotBeNull();
        document.Should().ContainKey(DatabaseConstants.PartitionKeysNames.Tenant);
        document[DatabaseConstants.PartitionKeysNames.Tenant]?.ToString().Should().Be(_tenantId.ToString());
    }
    ...
    public void Should_Create_New_Document_Instance_Each_Time()
    {
        ...
        // Act
        var document1 = _repository.PrepareCosmosDocument(entity, _tenantId);
        var document2 = _repository.PrepareCosmosDocument(entity, _tenantId);
        
        // Assert
        document1.Should().NotBeSameAs(document2);
        
        // But both should have the same content
        document1[DatabaseConstants.PartitionKeysNames.Tenant]?.ToString().Should().Be(document2[DatabaseConstants.PartitionKeysNames.Tenant]?.ToString());
        document1[GetJsonPropertyName(nameof(TestData.TestEntity.Id))]?.ToString()
            .Should().Be(document2[GetJsonPropertyName(nameof(TestData.TestEntity.Id))]?.ToString());
    }

    [Fact]
    public void Should_Use_Different_TenantIds_Correctly()
    {
        // Arrange
        var entity = TestData.NewTestEntity;
        ...
        // Act
        var document1 = _repository.PrepareCosmosDocument(entity, tenantId1);
        var document2 = _repository.PrepareCosmosDocument(entity, tenantId2);
        
        // Assert
        document1[DatabaseConstants.PartitionKeysNames.Tenant]?.ToString().Should().Be(tenantId1.ToString());
        document2[DatabaseConstants.PartitionKeysNames.Tenant]?.ToString().Should().Be(tenantId2.ToString());
        
        // Entity data should be the same
        document1[GetJsonPropertyName(nameof(entity.Id))]?.ToString().Should().Be(document2[GetJsonPropertyName(nameof(entity.Id))]?.ToString());
    }
